 ...
 *
 * Deskly's target deployment is a single Node process (the macOS app, or one
...
const globalForBus = globalThis as unknown as { desklyBus?: EventEmitter };

const bus =
  globalForBus.desklyBus ??
  (() => {
    const e = new EventEmitter();
    // One listener per connected browser tab; the default cap of 10 is far too low.
    e.setMaxListeners(0);
    globalForBus.desklyBus = e;
    return e;
  })();
