/**
 * Records a narrated walkthrough of Deskly as a video file.
 *
 *   npm run demo:video
 *
...
try {
  // --- 1. Sign in ------------------------------------------------------
  await goto(`${BASE}/login`, "Deskly", "Customer support ticketing for a small team");

  await page.fill('input[name="email"]', OWNER_EMAIL);
  await pause(400);
  await page.fill('input[name="password"]', "deskly123");
  ...
  await page.evaluate(
    ...
    [BASE, process.env.INBOUND_WEBHOOK_SECRET ?? "dev-inbound-secret", process.env.EMAIL_DOMAIN ?? "deskly.local", subject],
  );
  ...
  await step("Internal notes stay internal", "@mention a teammate — the customer never sees it", async () => {
    ...
    await note.click();
    await note.type("@sara@acme.test can you confirm the reversal cleared on Friday?", { delay: 16 });
  });
  await pause(900);
