...
1. **The queue.** Open `/inbox`. Twenty tickets across every status, each with
   ...
   shortcut.
2. **Email in.** Run
   `npm run simulate:email -- --to support@acme.deskly.local --from rahul@northwind.test --subject "Invoice 2291 charged twice"`.
...
5. **Internal note.** Press `n`, type `@sara@acme.test` and post. It renders
