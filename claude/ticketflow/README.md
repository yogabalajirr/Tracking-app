...
Sign in as **yogabalajirr@gmail.com** / **ticketflow123**.

For the desktop app:

```bash
npm run desktop      # build + launch
npm run dist:mac     # dist/TicketFlow-<version>-{arm64,x64}.dmg
```

`dist:mac` only runs on macOS — electron-builder shells out to `sips`,
`hdiutil` and `codesign`. Without a Mac, run the
**TicketFlow - Build macOS dmg** workflow in GitHub Actions; it builds on a macOS
...
`INBOUND_WEBHOOK_SECRET` (as `?secret=` or the `X-TicketFlow-Secret` header).
...
```html
<script src="https://your-ticketflow-host/widget.js"
