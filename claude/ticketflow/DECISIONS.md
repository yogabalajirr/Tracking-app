...
TicketFlow's shape is a single Node process — the macOS app, or one container. For
...
- **The `.dmg` has not been built.** Packaging is verified on Linux for both
  ...
  `TicketFlow.app` for arm64 and x64 before failing: the dmg step needs `sips` and
  `hdiutil`, and signing needs `codesign`. All three are macOS-only, so there
  is no workaround on Linux. The `TicketFlow - Build macOS dmg` GitHub Actions
